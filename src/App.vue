...
const processCrontab = (): void => {
  ...
  for (const line of lines) {
    ...
    const isIgnored =
      result.error === "Linha de comentário ignorada" ||
      result.error === "Linha vazia ignorada";
    if (!isIgnored) {
...
const onDrop = (dropIndex: number): void => {
  const isInvalidDrop =
    draggedIndex.value === null || draggedIndex.value === dropIndex;
...
<template>
  <main class="max-w-7xl mx-auto flex flex-col gap-10 pb-12 pt-8">
    <header
      class="flex flex-col gap-2 border-b border-neon-border pb-6 px-4 xl:px-0"
    >
      <h1 class="text-4xl font-extrabold text-white flex items-center gap-3">
        <span class="drop-shadow-[0_0_8px_rgba(14,165,233,0.8)]">
          <img
            src="/logo.svg"
            alt="Crontab Manager Logo"
      ...
      <p class="text-slate-400 text-lg">
        Decodifique, gerencie e orquestre tarefas do sistema de forma visual e
        intuitiva.
      </p>
    </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-8 px-4 xl:px-0">
      ...
      <div class="flex flex-col gap-8">
        <section
          class="flex flex-col gap-3 bg-neon-surface p-6 rounded-2xl border border-neon-border shadow-lg"
        >
          <div class="flex flex-col gap-1">
            <h2 class="text-lg font-bold text-white">Importação em Massa</h2>
            <p class="text-sm text-slate-500 mb-2">
              Cole o seu crontab atual abaixo para processar e gerar os cartões
          ...
          <div class="flex justify-end mt-2">
            <button
              @click="processCrontab"
              class="bg-neon-accent text-white px-6 py-3 rounded-xl font-bold hover:bg-sky-400 shadow-neon transition-colors cursor-pointer"
            >
              Processar Tarefas
            </button>
          </div>
        ...
        <section
          ...
          class="bg-neon-error/10 border border-neon-error/30 rounded-2xl p-6 shadow-lg"
        >
          <h2
            class="text-neon-error font-bold mb-3 flex items-center gap-2 text-lg"
          >
            ⚠ Linhas Ignoradas ou com Erro
          </h2>
          <ul
            class="flex flex-col gap-2 text-sm text-neon-error/90 font-mono w-full"
          >
            <li
              v-for="(err, index) in parseErrors"
              :key="index"
              class="bg-black/30 p-3 rounded-lg border border-neon-error/20 break-all overflow-hidden"
            >
              {{ err }}
            </li>
          </ul>
        ...
        <section class="flex flex-col gap-5">
          ...
          <div
            ...
            class="flex flex-col items-center justify-center p-12 border-2 border-dashed border-neon-border rounded-2xl bg-neon-surface/50 text-slate-500"
          >
